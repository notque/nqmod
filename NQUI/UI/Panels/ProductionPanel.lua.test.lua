...
function PopulateList(data, listMode, listIM)
	...
	if(listMode == LISTMODE.PRODUCTION) then
		...
		-- check for a placed district
		local districtplaced = false;
		--local PlacedHash = 0;
		
		local pBuildQueue = cityData:GetBuildQueue();

		for i, item in ipairs(data.DistrictItems) do
			if (pBuildQueue:HasBeenPlaced(item.Hash)) then
                districtplaced = true;
            end
		end
		...
		for i, item in ipairs(data.DistrictItems) do
			...
			--if (bDistrictIsPlaced and not (item.Hash == PlacedHash) then
			--	item.Disabled = true;
			--end
